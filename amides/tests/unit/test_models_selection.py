...
def train_data():
    ...
    return DataBunch(
        np.array(train_data_transformed), np.array([0, 1, 1, 0]), ["benign", "matching"]
    )
...
def train_test_split():
    ...
    test_data = DataBunch(
        test_data_transformed, np.array([1, 0, 1, 0]), ["benign", "matching"]
    )
    train_data = DataBunch(
        train_data_transformed, np.array([0, 1, 1, 0]), ["benign", "matching"]
    )
...
class TestHParamOptimizer:
    ...
    def test_search_best_parameters(self):
        optimizer = HyperParameterOptimizer(
            estimator=SVC(),
            param_grid={
                "C": np.logspace(-1, 1, num=1),
    ...
    def test_search_best_parameters_invalid_cv_schema(self):
        optimizer = HyperParameterOptimizer(
            estimator=SVC(),
            param_grid={
                "C": np.logspace(-1, 1, num=1),
    ...
    def test_search_and_predict(self, hp_optimizer):
